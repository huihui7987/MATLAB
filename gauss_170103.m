...
gauss_spec=fftshift(fft((gauss_time)));    %傅里叶变换，并且进行移位操作。
...
%%1阶理想
figure;
idea_t_c = ifft(idea_gauss_diff,Ns);
plot(t*1e+9,abs(idea_t_c),'linewidth',2.5);
figure;%基本认为是对的
test = ifft(ff_c,Ns);
plot(t*1e+9,abs(test),'linewidth',2.5);
